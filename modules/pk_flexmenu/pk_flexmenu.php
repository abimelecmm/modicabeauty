	...
	private function makeMenuOption($id_menu = NULL, $panel = NULL) {
		...
		foreach ($menu_item as $item) {
			...
			switch (substr($item, 0, strlen($values[1]))) {
				...
				case 'PSL' :	
					if ($id == 0)
						$this->_html .= '<li data-value="PSL0">'.$this->l('Home').'</li>';
					if ($id == 1)
						$this->_html .= '<li data-value="PSL1">'.$this->l('Specials').'</li>';
					if ($id == 2)
						$this->_html .= '<li data-value="PSL2">'.$this->l('New products').'</li>';
					if ($id == 3)
						$this->_html .= '<li data-value="PSL3">'.$this->l('Best sellers').'</li>';
					if ($id == 4)
						$this->_html .= '<li data-value="PSL4">'.$this->l('Our stores').'</li>';
					if ($id == 5)
						$this->_html .= '<li data-value="PSL5">'.$this->l('Contact us').'</li>';
					if ($id == 6)
						$this->_html .= '<li data-value="PSL6">'.$this->l('Sitemap').'</li>';					
					break;
