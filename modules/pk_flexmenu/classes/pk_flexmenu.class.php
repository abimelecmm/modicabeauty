...
class FlexMenuItem
{
	...
	public static function updateLink($link, $labels, $newWindow = 0, $id_shop, $id_link) {
		...
		foreach ($labels as $id_lang => $label)
			Db::getInstance()->update('pk_flexmenu_links_lang', array('id_shop' => (int)$id_shop, 'label' => pSQL($label), 'link' => pSQL($link[$id_lang])), 'id_pk_flexmenu_links = ' . (int)$id_link . ' AND id_lang = ' . (int)$id_lang);
	}
