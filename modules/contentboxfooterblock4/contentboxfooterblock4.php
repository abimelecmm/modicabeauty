...
class CONTENTBOXFOOTERBLOCK4 extends Module
{
	public function __construct()
	{
		$this->name = 'contentboxfooterblock4';
		$this->description = 'Contentbox - Footer Block #4 (rightmost)';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Contentboxfooterblock4_MONOLANGUAGE';
		$this->text_editor_content = 'Contentboxfooterblock4_TEXTEDITOR';
		$this->content_wrapper = 'Contentboxfooterblock4_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Contentboxfooterblock4_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Contentboxfooterblock4_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Contentbox - Footer Block #4');
		$this->description = $this->l('Contentbox - Footer Block #4 (rightmost)');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !CONTENTBOXFOOTERBLOCK4Model::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !CONTENTBOXFOOTERBLOCK4Model::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = CONTENTBOXFOOTERBLOCK4Model::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = CONTENTBOXFOOTERBLOCK4Model::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					CONTENTBOXFOOTERBLOCK4Model::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					CONTENTBOXFOOTERBLOCK4Model::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																CONTENTBOXFOOTERBLOCK4Model::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class CONTENTBOXFOOTERBLOCK4Model extends ObjectModel
{

	public static $definition = array(
		'table' => 'contentboxfooterblock4',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( CONTENTBOXFOOTERBLOCK4Model::createFilesTable()
				&& CONTENTBOXFOOTERBLOCK4Model::createContentTable());
	}
