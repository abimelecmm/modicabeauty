...
class LengowImportAbstract {
	 ...
	 *
	 */
	protected static function importOrders($orders, $id_lang, $id_shop, $debug = false, $force_product = true, $limit = 0, $log_output = false, $force_price = true)
	{
	...
	protected static function getProducts($cart_data, $marketplace, $lengow_id, $id_shop)
	{
		...
		foreach ($cart_data->products->product as $product)
		{
			...
			if (!empty($product_data['status']))
				...
				{
					LengowCore::log('product '.$product_data['sku'].' could not be added to cart - status: '.$marketplace->getStateLengow((string)$product_data['status']), false, $lengow_id);
					continue;
				}
			$ids = false;
			$product_ids = array(
	 ...
	 * @return mixed
	 */
	public static function getCarrierId($tracking_informations, LengowMarketplace $marketplace, $lengow_id, $id_lang, $shipping_address)
	{
		$carrier_id = false;
		...
		else
		{
			...
			if ($carrier_id == Configuration::get('SOCOLISSIMO_CARRIER_ID'))
			{
				...
				{
					$carrier_id = (int)Configuration::get('LENGOW_IMPORT_CARRIER_DEFAULT');
					LengowCore::log('module version '.$carrier->external_module_name.' not supported. Default carrier assigned.', false, $lengow_id);
				}
			}
