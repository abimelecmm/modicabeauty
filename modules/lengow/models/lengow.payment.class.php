...
class LengowPaymentModuleAbstract extends PaymentModule
{
	 ...
	 */
	public function makeOrder($id_cart, $id_order_state, $amount_paid, $payment_method, $message, $lengow_products, $lengow_shipping_costs, $processing_fees = null)
	{
		...
		foreach ($package_list as $id_address => $packageByAddress)
			...
			foreach ($packageByAddress as $id_package => $package)
			{
				...
				$order->total_paid_tax_excl = (float)Tools::ps_round((float)$total_products + (float)$total_shipping_tax_excl + (float)$order->total_wrapping_tax_excl, $precision);
				$order->total_paid_tax_incl = (float)Tools::ps_round((float)$total_products_wt + (float)$order->total_shipping_tax_incl + (float)$processing_fees, $precision);
	...
	public function makeOrder14($id_cart, $id_order_state, $amount_paid, $payment_method, $message, $lengow_products, $lengow_shipping_costs, $processing_fees = null)
	{
		if (!isset($this->context))
			$this->context = Context::getContext();
		$this->context->cart = new Cart($id_cart);
