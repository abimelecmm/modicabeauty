...
class LengowOrderAbstract extends Order {
	...
	*/
	public static function getByOrderIDFlux($id_order_lengow, $id_flux)
	{
		$query = 'SELECT `id_order` FROM `'._DB_PREFIX_.'lengow_orders` '
				.'WHERE `id_order_lengow` = \''.pSQL($id_order_lengow).'\' '
				.'AND `id_flux` = \''.(int)$id_flux.'\' LIMIT 1;';
	...
	*/
	...
	{
		$update = 'UPDATE `'._DB_PREFIX_.'order_payment` SET `amount` = \''.(float)$this->total_paid.'\' WHERE `order_reference` = \''.pSQL($this->reference).'\' LIMIT 1;';
	...
	*/
	public function rebuildOrderCarrier()
	{
		...
		if ($id_order_carrier)
		{
			...
			$order_carrier->shipping_cost_tax_exc = (float)$this->total_shipping_tax_excl;
			$order_carrier->shipping_cost_tax_incl = (float)$this->total_shipping_tax_incl;
