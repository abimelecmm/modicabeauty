...
<form id="_form" class="defaultForm lengow" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}" method="post" enctype="multipart/form-data" >
	<fieldset id="fieldset_0">
		...
		<div class="margin-form">
			{$checklist|escape:'htmlall':'UTF-8'}
		</div>
		<div class="clear"></div>
	</fieldset>
	<br />
	<fieldset id="fieldset_1">
		...
		<div class="margin-form">
		{$help_credentials|escape:'htmlall':'UTF-8'}
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_tracking" class="" id="lengow_tracking">
				{foreach from=$options.trackers item=option}
					<option value="{$option->id|escape:'htmlall':'UTF-8'}"{if $option->id == $lengow_tracking} selected="selected"{/if}>{$option->name|escape:'htmlall'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_image_type" class="" id="lengow_image_type">
				{foreach from=$options.images item=option}
					<option value="{$option.id_image_type}"{if $option.id_image_type == $lengow_image_type} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_images_count" class="" id="lengow_images_count">
				{foreach from=$options.images_count item=option}
					<option value="{$option->id}"{if $option->id == $lengow_images_count} selected="selected"{/if}>{$option->name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_export_format" class="" id="lengow_export_format">
				{foreach from=$options.formats item=option}
					<option value="{$option->id}"{if $option->id == $lengow_export_format} selected="selected"{/if}>{$option->name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_export_fields[]" class="lengow-select" size="15" multiple="multiple">
				{foreach from=$options.export_fields item=field}
					<option value="{$field->id}"{if $field->id|in_array:$lengow_export_fields} selected="selected"{/if}>{$field->name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
			<p class="preference_description">{l s='Maintain "control key or command key" to select fields.' mod='lengow'}{$link_file_export|escape:'htmlall':'UTF-8'}</p>
		</div>
		<div class="clear"></div>
		<label>{l s='Your export script' mod='lengow'}</label>
		<div class="margin-form">
			{$url_feed_export|escape:'htmlall':'UTF-8'}
		</div>
		<div class="clear"></div>
		<div class="small"><sup>*</sup> {l s='Required field' mod='lengow'}</div>
	</fieldset>
	...
	<fieldset id="fieldset_6"> <legend>{l s='Import parameters' mod='lengow'}</legend>
		...
		<div class="margin-form">
			<select name="lengow_order_process" class="" id="lengow_order_process">
				{foreach from=$options.states item=option}
					<option value="{$option.id_order_state}"{if $option.id_order_state == $lengow_order_process} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_order_shipped" class="" id="lengow_order_shipped">
				{foreach from=$options.states item=option}
					<option value="{$option.id_order_state}"{if $option.id_order_state == $lengow_order_shipped} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_order_cancel" class="" id="lengow_order_cancel">
				{foreach from=$options.states item=option}
					<option value="{$option.id_order_state}"{if $option.id_order_state == $lengow_order_cancel} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_order_shippedByMp" class="" id="lengow_order_shippedByMp">
				{foreach from=$options.states item=option}
					<option value="{$option.id_order_state}"{if $option.id_order_state == $lengow_order_shippedByMp} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_method_name" class="" id="lengow_method_name">
				{foreach from=$options.shippings item=option}
					<option value="{$option->id}"{if $option->id == $lengow_method_name} selected="selected"{/if}>{$option->name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		</div>
		<div class="clear"></div>
		...
		<div class="margin-form">
			<select name="lengow_carrier_default" class="" id="lengow_carrier_default">
				{foreach from=$options.carriers item=option}
					<option value="{$option.id_carrier}"{if $option.id_carrier == $lengow_carrier_default} selected="selected"{/if}>{$option.name|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
			</select>
		...
		<div class="margin-form">
			...
			<p class="preference_description">{l s='Please note that stock will be decremented. Maintain "control key or command key" to select multiple marketplaces.' mod='lengow'}{$link_file_export|escape:'htmlall':'UTF-8'}</p>
		</div>
		...
		<div class="margin-form">
			{$lengow_is_import|escape:'htmlall':'UTF-8'}
		</div>
		<div class="clear"></div>
		<label>{l s='Your import script' mod='lengow'}</label>
		<div class="margin-form">
			{$url_feed_import|escape:'htmlall':'UTF-8'}
		</div>
		<div class="clear"></div>
		<div class="small"><sup>*</sup> {l s='Required field' mod='lengow'}</div>
	</fieldset>
	...
	<fieldset id="fieldset_7"><legend>{l s='Cron' mod='lengow'}</legend>
		{$lengow_cron|escape:'htmlall':'UTF-8'}
		<div class="clear"></div>
	</fieldset>
	<br />
	<fieldset id="fieldset_7"> <legend>{l s='Developer' mod='lengow'}</legend>
		...
		<div class="margin-form">
			{$log_files|escape:'htmlall':'UTF-8'}
		</div>
		<div class="margin-form">
