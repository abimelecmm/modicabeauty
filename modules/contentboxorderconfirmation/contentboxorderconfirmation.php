...
class CONTENTBOXORDERCONFIRMATION extends Module
{
	public function __construct()
	{
		$this->name = 'contentboxorderconfirmation';
		$this->description = 'Content that will be added to the checkout payment confirmation page';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Contentboxorderconfirmation_MONOLANGUAGE';
		$this->text_editor_content = 'Contentboxorderconfirmation_TEXTEDITOR';
		$this->content_wrapper = 'Contentboxorderconfirmation_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Contentboxorderconfirmation_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Contentboxorderconfirmation_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Contentbox - order confirmation');
		$this->description = $this->l('Content that will be added to the checkout payment confirmation page');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !CONTENTBOXORDERCONFIRMATIONModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !CONTENTBOXORDERCONFIRMATIONModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = CONTENTBOXORDERCONFIRMATIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = CONTENTBOXORDERCONFIRMATIONModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					CONTENTBOXORDERCONFIRMATIONModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					CONTENTBOXORDERCONFIRMATIONModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																CONTENTBOXORDERCONFIRMATIONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class CONTENTBOXORDERCONFIRMATIONModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'contentboxorderconfirmation',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( CONTENTBOXORDERCONFIRMATIONModel::createFilesTable()
				&& CONTENTBOXORDERCONFIRMATIONModel::createContentTable());
	}
