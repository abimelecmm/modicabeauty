...
class VertFlexMenuItem
{
	public static function getCustomLink($id_linksmenutop, $id_lang, $id_shop) {		
		$sql = 'SELECT l.id_pk_vertflexmenu_links, l.id_lang, l.id_shop, l.label, l.link, ll.new_window FROM '._DB_PREFIX_.'pk_vertflexmenu_links_lang l
				LEFT JOIN '._DB_PREFIX_.'pk_vertflexmenu_links ll ON (l.id_pk_vertflexmenu_links = ll.id_pk_vertflexmenu_links AND ll.id_shop='.(int)$id_shop.') WHERE l.id_pk_vertflexmenu_links='.$id_linksmenutop.' AND l.id_lang = '.(int)$id_lang.' AND l.id_shop='.$id_shop;
		return Db::getInstance()->executeS($sql);
	}
	
	public static function gets($id_lang, $id_menu = null, $id_shop)
	{
		$sql = 'SELECT l.id_pk_vertflexmenu_menus, l.position, l.active, ll.label
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus l
				LEFT JOIN '._DB_PREFIX_.'pk_vertflexmenu_menus_lang ll ON (l.id_pk_vertflexmenu_menus = ll.id_pk_vertflexmenu_menus AND ll.id_shop='.(int)$id_shop.')
				WHERE 1 '.((!is_null($id_menu)) ? ' AND l.id_pk_vertflexmenu_menus = "'.(int)$id_menu.'"' : '').'
	...
	public static function get_last_position($id_shop)
	{
		$sql = 'SELECT MAX(`position`) as max_position
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus WHERE id_shop IN (0, '.(int)$id_shop.')';
	...
	{
		$sql = 'SELECT *
				FROM '._DB_PREFIX_.'pk_vertflexmenu_submenus l
	...
	{
		$sql = 'SELECT position
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus WHERE 1 '.((!is_null($id_menu)) ? ' AND id_pk_vertflexmenu_menus = "'.(int)$id_menu.'"' : '').'
				AND id_shop IN (0, '.(int)$id_shop.')';
		
		$curr_position = Db::getInstance()->getValue($sql);
		
		$sql = 'SELECT MAX(`position`)
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus  WHERE position < '.(int)$curr_position.' AND id_shop IN (0, '.(int)$id_shop.')';
		$prev_position = Db::getInstance()->getValue($sql);
		
		$sql = 'SELECT id_pk_vertflexmenu_menus
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus  WHERE position = '.(int)$prev_position.' AND id_shop IN (0, '.(int)$id_shop.')';
		$prev_menu_id = Db::getInstance()->getValue($sql);
		
		if(isset($prev_position) && isset($prev_menu_id)){
		Db::getInstance()->update(
			'pk_vertflexmenu_menus',
			array(
				'position'=>(int)$prev_position,
				'id_shop' => (int)$id_shop
			),
			'id_pk_vertflexmenu_menus = '.(int)$id_menu
		);
		...
		Db::getInstance()->update(
			...
			'id_pk_vertflexmenu_menus = '.(int)$prev_menu_id
		);
	...
	public static function moveDown($id_menu, $id_shop)
	{
		...
		$sql = 'SELECT id_pk_vertflexmenu_menus
				FROM '._DB_PREFIX_.'pk_vertflexmenu_menus  WHERE position = '.(int)$next_position.' AND id_shop IN (0, '.(int)$id_shop.')';
		...
		Db::getInstance()->update(
			...
			'id_pk_vertflexmenu_menus = '.(int)$next_menu_id
		);
 	...
 	public static function getLinkLang($id_menu, $id_shop)
	{
		$ret = Db::getInstance()->executeS('
			SELECT l.id_pk_vertflexmenu_menus, l.position, l.active, l.narrow, l.main_image, ll.label
			FROM '._DB_PREFIX_.'pk_vertflexmenu_menus l
			LEFT JOIN '._DB_PREFIX_.'pk_vertflexmenu_menus_lang ll ON (l.id_pk_vertflexmenu_menus = ll.id_pk_vertflexmenu_menus AND ll.id_shop='.(int)$id_shop.')
			WHERE 1
			'.((!is_null($id_menu)) ? ' AND l.id_pk_vertflexmenu_menus = "'.(int)$id_menu.'"' : '').'
	...
	public static function getCustomLinkLang($id_linksmenutop, $id_shop) {
		$ret = Db::getInstance() -> executeS('
			SELECT l.id_pk_vertflexmenu_links, l.new_window, ll.link, ll.label, ll.id_lang
			FROM ' . _DB_PREFIX_ . 'pk_vertflexmenu_links l
			LEFT JOIN ' . _DB_PREFIX_ . 'pk_vertflexmenu_links_lang ll ON (l.id_pk_vertflexmenu_links = ll.id_pk_vertflexmenu_links AND ll.id_shop=' . (int)$id_shop . ')
			WHERE 1
			' . ((!is_null($id_linksmenutop)) ? ' AND l.id_pk_vertflexmenu_links = "' . (int)$id_linksmenutop . '"' : '') . '
			AND l.id_shop IN (0, ' . (int)$id_shop . ')
		');
	...
	public static function ajaxAddName($id_menu, $label, $id_shop, $id_lang)
	{
		...
		if ($id_menu == "NONE") {
			...
			$sql = 'SELECT COUNT(*)	FROM `'._DB_PREFIX_.'pk_vertflexmenu_menus_lang` WHERE `id_pk_vertflexmenu_menus` = '.(int)$id_menu.' AND id_shop = '.(int)$id_shop;
			...
			if ($res) {
				Db::getInstance()->update(
					...
					'id_pk_vertflexmenu_menus = '.(int)$id_menu.' AND id_shop = '.(int)$id_shop
				);		
			} else {
				Db::getInstance()->insert(
					'pk_vertflexmenu_menus_lang',
	...
	/*public static function ajaxAddLink($id_menu, $link, $id_lang, $id_shop)
	{
		...
		$sql = 'SELECT COUNT(*)	FROM `'._DB_PREFIX_.'pk_vertflexmenu_menus_lang` WHERE `id_menu` = '.(int)$id_menu.' AND id_lang = '.(int)$id_lang.' AND id_shop = '.(int)$id_shop;
		$res = Db::getInstance()->getValue($sql);
	...
	{
		Db::getInstance()->delete('pk_vertflexmenu_menus', 'id_pk_vertflexmenu_menus = '.(int)$id_menu);
		Db::getInstance()->delete('pk_vertflexmenu_menus_lang', 'id_pk_vertflexmenu_menus = '.(int)$id_menu);
		Db::getInstance()->delete('pk_vertflexmenu_submenus', 'id_menu = '.(int)$id_menu);
	}

	public static function getLinks($id_lang, $id_linksmenutop = null, $id_shop) {

		$sql = 'SELECT l.id_pk_vertflexmenu_links, l.new_window, ll.link, ll.label
				FROM ' . _DB_PREFIX_ . 'pk_vertflexmenu_links l
				LEFT JOIN ' . _DB_PREFIX_ . 'pk_vertflexmenu_links_lang ll ON (l.id_pk_vertflexmenu_links = ll.id_pk_vertflexmenu_links AND ll.id_lang = ' . (int)$id_lang . ' AND ll.id_shop=' . (int)$id_shop . ')
				WHERE 1 ' . ((!is_null($id_linksmenutop)) ? ' AND l.id_pk_vertflexmenu_links = "' . (int)$id_linksmenutop . '"' : '') . '
				AND l.id_shop IN (0, ' . (int)$id_shop . ')';
		return Db::getInstance()->executeS($sql);
	}
	...
	public static function addLink($link, $label, $newWindow = 0, $id_shop) {
		...
		Db::getInstance()->insert('pk_vertflexmenu_links', array('new_window' => (int)$newWindow, 'id_shop' => (int)$id_shop));
		$id_linksmenutop = Db::getInstance()->Insert_ID();

		foreach ($label as $id_lang => $label)
			Db::getInstance()->insert('pk_vertflexmenu_links_lang', array('id_pk_vertflexmenu_links' => (int)$id_linksmenutop, 'id_lang' => (int)$id_lang, 'id_shop' => (int)$id_shop, 'label' => pSQL($label), 'link' => pSQL($link[$id_lang])));
	}
	...
	public static function updateLink($link, $labels, $newWindow = 0, $id_shop, $id_link) {
		...
		Db::getInstance()->update('pk_vertflexmenu_links', array('new_window' => (int)$newWindow, 'id_shop' => (int)$id_shop), 'id_pk_vertflexmenu_links = '.(int)$id_link);

		foreach ($labels as $id_lang => $label)
			Db::getInstance()->update('pk_vertflexmenu_links_lang', array('id_shop' => (int)$id_shop, 'label' => pSQL($label), 'link' => pSQL($link[$id_lang])), 'id_pk_vertflexmenu_links = ' . (int)$id_link . ' AND id_lang = ' . (int)$id_lang);
	}

	public static function removeLink($id_linksmenutop, $id_shop) {
		Db::getInstance()->delete('pk_vertflexmenu_links', 'id_pk_vertflexmenu_links = ' . (int)$id_linksmenutop . ' AND id_shop = ' . (int)$id_shop);
		Db::getInstance()->delete('pk_vertflexmenu_links_lang', 'id_pk_vertflexmenu_links = ' . (int)$id_linksmenutop);
	}
	...
	public static function updateSectionData($section, $data, $menuID) {
		...
		if ($section == "main_image") {
			$table = "pk_vertflexmenu_menus";
			$column = "id_pk_vertflexmenu_menus";
		} else {
			$table = "pk_vertflexmenu_submenus";
			$column = "id_menu";
		}
	...
	public static function removeImage($field, $menuID) {
		...
		if ($res == true) unlink(_PS_MODULE_DIR_."pk_vertflexmenu/uploads/".$imgname[0][$field."_image"]);
	}
	
	public static function removeItem($field, $menuID, $item) {

		$sql = 'SELECT '.$field.' FROM '._DB_PREFIX_.'pk_vertflexmenu_submenus WHERE id_menu='.(int)$menuID;
