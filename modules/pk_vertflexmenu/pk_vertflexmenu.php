...
require (dirname(__FILE__).'/classes/pk_vertflexmenu.class.php');

class pk_vertflexmenu extends Module {
	...
	public $imageSuffix = "alysum";

	public function __construct() {
		$this->name = 'pk_vertflexmenu';
		...
		parent::__construct();
		$this->displayName = $this->l('Vertical FlexMenu');
		$this->description = $this->l('Venedor Vertical Flexible Menu');
	}
	...
	private function uninstallDb() {
        ...
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'quick_access_lang` WHERE name = "Venedor Menu"');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'pk_vertflexmenu_menus`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'pk_vertflexmenu_submenus`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'pk_vertflexmenu_menus_lang`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'pk_vertflexmenu_links`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'pk_vertflexmenu_links_lang`');
		return true;
	}
	...
	public function getContent() {
		...
		if (Tools::isSubmit('submitMenuTabRemove')) {
			$id_menu = Tools::getValue('id_pk_vertflexmenu_menus', 0);
			VertFlexMenuItem::remove($id_menu, (int)Shop::getContextShopID());
			...
			$id_menu = Tools::getValue('id_pk_vertflexmenu_menus', 0);
			VertFlexMenuItem::moveUp($id_menu, (int)Shop::getContextShopID());
			$this->_html .= $this->displayConfirmation($this->l('Menu order changed'));
		} else if (Tools::isSubmit('moveDown')) {
			$id_menu = Tools::getValue('id_pk_vertflexmenu_menus', 0);
			VertFlexMenuItem::moveDown($id_menu, (int)Shop::getContextShopID());
			$this->_html .= $this->displayConfirmation($this->l('Menu order changed'));
			...
			if ((!count($links_label)) && (!count($labels)));
				...
				VertFlexMenuItem::addLink($links_label, $labels, Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
				$this->_html .= $this->displayConfirmation($this->l('The link has been added'));
			}
			$update_cache = true;
		} else if (Tools::isSubmit('removeLink')) {
			$id_pk_vertflexmenu_links = Tools::getValue('id_pk_vertflexmenu_links', 0);
			VertFlexMenuItem::removeLink($id_pk_vertflexmenu_links, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKTOPMENU_ITEMS', str_replace(array('LNK' . $id_pk_vertflexmenu_links . ',', 'LNK' . $id_pk_vertflexmenu_links), '', Configuration::get('MOD_BLOCKTOPMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		} else if (Tools::isSubmit('editLink')) {
			$id_pk_vertflexmenu_links = (int)Tools::getValue('id_pk_vertflexmenu_links', 0);
			$id_shop = (int)Shop::getContextShopID();

			if (!Tools::isSubmit('link')) {				
				$link_params = VertFlexMenuItem::getCustomLinkLang($id_pk_vertflexmenu_links, $id_shop);
			} else {
				VertFlexMenuItem::updateLink(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_pk_vertflexmenu_links, (int)$id_pk_vertflexmenu_links);
		...
		$links = VertFlexMenuItem::gets((int)$id_lang, null, (int)Shop::getContextShopID());
					...
					<div class="list_menuitem">
						'.(isset($link['active']) && ($link['active'] == 1) ? '<img src="'.$this->_path.'img/act.png" />' : ' <img src="'.$this->_path.'img/pas.png" />').'<span>'.Tools::safeOutput($itemData['name']).'</span>
							<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
								<input type="hidden" name="id_pk_vertflexmenu_menus" id="id_menu" value="'.(int)$link['id_pk_vertflexmenu_menus'].'" />
		...
		<fieldset class="menuItem" data-shop-id="'.(int)Shop::getContextShopID().'">
			...
			$links = VertFlexMenuItem::getLinks((int)$id_lang, null, (int)Shop::getContextShopID());		
			...
			if (count($links)) {
				...
				<div class="list_menuitem customlinks">
					...
					<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
						<input type="hidden" name="id_pk_vertflexmenu_links" value="'.(int)$link['id_pk_vertflexmenu_links'].'" />
			...
			$this->_html .= '<label style="clear: both;">'.$this->l('New Window').'</label>
				...
				<div class="margin-form">';
					if (Tools::isSubmit('id_pk_vertflexmenu_links'))
						$this->_html .= '<input type="hidden" name="id_pk_vertflexmenu_links" value="'.(int)Tools::getValue('id_pk_vertflexmenu_links').'" />';
	...
	public function displayAddTabForm($id_menu = NULL) {
		...
		if (isset($id_menu)) {
			if (!Tools::isSubmit('link')) {
				$link_params = VertFlexMenuItem::getLinkLang($id_menu, $id_shop);				
				$section_settings = VertFlexMenuItem::getinside($id_menu);				
			}
		}
		...
		$this->_html .= '<fieldset class="menuItem" data-shop-id="'.(int)Shop::getContextShopID().'">';
			...
			if (isset($id_menu)) {
				$this->_html .= '<input type="hidden" name="id_pk_vertflexmenu_menus" id="id_menu" value="'.$id_menu.'" />';
			} else {
				$this->_html .= '<input type="hidden" name="id_pk_vertflexmenu_menus" id="id_menu" value="NONE" />';
			}
			...
			<div class="margin-form">
				<select id="topLevelLinks" data-lang="'.$id_lang.'" style="height:30px">';
					...
					// BEGIN Blog
					$links = VertFlexMenuItem::getBlogLinks($id_lang);
					if ($links != false) {
					...
					$links = VertFlexMenuItem::getLinks($id_lang, null, (int)Shop::getContextShopID());
					foreach ($links as $link) {
						if ($link['label'] == '') {
							$link = VertFlexMenuItem::getLink($link['id_pk_vertflexmenu_links'], $default_language, (int)Shop::getContextShopID());
							$this->_html .= '<option '.(isset($link_params['label']) ? ($link_params['label'] == 'LNK'.(int)$link[0]['id_pk_vertflexmenu_links'] ? 'selected' : '') : '').' value="LNK'.(int)$link[0]['id_pk_vertflexmenu_links'].'">'.$spacer.$link[0]['label'].'</option>';
						} else
							$this->_html .= '<option '.(isset($link_params['label']) ? ($link_params['label'] == 'LNK'.(int)$link['id_pk_vertflexmenu_links'] ? 'selected' : '') : '').' value="LNK'.(int)$link['id_pk_vertflexmenu_links'].'">'.$spacer.$link['label'].'</option>';
					}
					$this->_html .= '</optgroup>';
					...
					$.ajaxSetup({
				        type: "POST",
				        url: "'.Tools::getShopProtocol().Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri.'modules/pk_vertflexmenu/ajax/queries.php",
								...
								<div id="mainlinks_links" class="margin_field '.(isset($section_settings["main"]) && ($section_settings["main"] == "LINKS") ? '' : 'hide').'" data-type-content="main-links">
									...
									<div class="shop_source">										
										...
										foreach ($links as $link) {
											if ($link['label'] == '') {
												$link = VertFlexMenuItem::getLink($link['id_pk_vertflexmenu_links'], $default_language, (int)Shop::getContextShopID());
												$this->_html .= '<option value="LNK'.(int)$link[0]['id_pk_vertflexmenu_links'].'">'.$spacer.$link[0]['label'].'</option>';
											} else
												$this->_html .= '<option value="LNK'.(int)$link['id_pk_vertflexmenu_links'].'">'.$spacer.$link['label'].'</option>';
										}
										$this->_html .= '</optgroup>';
										$this->_html .= '</select>
									</div>
	...
	private function makeMenuOption($id_menu = NULL, $panel = NULL) {
		...
		foreach ($menu_item as $item) {
			...
			switch (substr($item, 0, strlen($values[1]))) {
				...
				case 'BLG' :
					$links = VertFlexMenuItem::getBlogLink($id_lang, (int)$id);
					if ($links != false) {
						foreach ($links as $link) {
				...
				case 'LNK' :
					$link = VertFlexMenuItem::getCustomLink((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link)) {
						if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = VertFlexMenuItem::getCustomLink($link[0]['id_pk_vertflexmenu_links'], (int)$default_language, (int)Shop::getContextShopID());							
						}
						$this->_html .= '<li data-value="LNK' . $link[0]['id_pk_vertflexmenu_links'] . '">' . $link[0]['label'] . '<span class="removeThis"></span></li>';
					}
					break;
				case 'SHOP' :
	...
	private function getMenuItems($id_menu = NULL, $panel = NULL) {
		...
		$sections = VertFlexMenuItem::getinside($id_menu);
		switch($panel) {
			case 'leftimage' :
	...
	private function getCategory($id_category, $depth = 0, $id_lang = false, $id_shop = false) {
		...
		if (!empty($is_intersected)) {
			...
			$this->_menu .= '<a class="menu-item-title" href="'.$category_link.'">'.$category->name.'</a>'.(count($children) ? '<a href="#" class="opener"><svg class="svgic svgic-account"><use xlink:href="#si-arrowdown"></use></svg></a>' : '');
			...
			switch ($type)
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = VertFlexMenuItem::getCustomLink($link[0]['id_pk_vertflexmenu_links'], $default_language, (int)Shop::getContextShopID());
						}
		...
		switch ($type)
		{
			...
			case 'BLG':
				...
				if ($id == 0) {										
					...
					$link = VertFlexMenuItem::getBlogLink($id_lang, $id);
					if ($link == false) 
						return false;
	...
	{	
		...
		$data = VertFlexMenuItem::getsfm($id_lang, $id_shop);	
		...
		foreach ($data as $link) {			
			...
			$ext = ".jpg";
			if ($itemData['catid']) {
				$icon = _PS_MODULE_DIR_."pk_themesettings/images/upload/".$itemData['catid']."-".$id_shop;
				if ((file_exists($icon.".jpg"))) {
					$icon = $icon.".jpg";
					$ext = ".jpg";
				}
				elseif ((file_exists($icon.".png"))) {
					$icon = $icon.".png";
					$ext = ".png";
				}
			}
			...
			if ((file_exists($icon)) && ($this->getOptions() == 1))
				$icon_html = '<img class="cat-icon" src="'.$shop->getBaseURI().'modules/pk_themesettings/images/upload/'.$itemData['catid'].'-'.$id_shop.$ext.'" alt="" />';
							
			$this->_menu .= '<li class="v-flexmenuitem id_menu'.$link['id_pk_vertflexmenu_menus'].(($submenu) ? ' hasChildren' : '').(($link['narrow'] == 1)  ? ' narrow' : '').(($this->curPath() == $itemData['link']) ? ' current-item' : '').'" data-menuid="'.$link['id_pk_vertflexmenu_menus'].'"><a href="'.$itemData['link'].'" '.(((!empty($itemData['new_window'])) && ($itemData['new_window'] == 1)) ? ' target="_blank"' : '').'>'.$icon_html.'<span>'.$itemData['name'].'</span></a>'.(($submenu) ? '<a href="#" class="opener"><svg class="svgic svgic-account"><use xlink:href="#si-arrowdown"></use></svg></a>' : '').'';		
			...
			$this->_menu .='<div class="submenu dd-section'.$column.' fmsid'.$link['id_pk_vertflexmenu_menus'].' clearfix"><div class="sections-contaier"'.(($link['main_image'] != "")  ? ' style="background-image:url('.$this->context->shop->getBaseURL().'modules/pk_vertflexmenu/uploads/'.$link['main_image'].')"' : '').'>';
					...
					switch ($link['bottom']) {
						...
						case 'LINKS':
							...
							if ($link['bottom_carousel'] == 1)
								$this->_menu .= '<script>$(".id_menu'.$link['id_pk_vertflexmenu_menus'].' .v-bottom-section-links").flexisel({pref: "vm-man",visibleItems: 5,animationSpeed: 500,autoPlay: true,autoPlaySpeed: 3900,pauseOnHover: true,enableResponsiveBreakpoints: true,clone : true, responsiveBreakpoints: {portrait: { changePoint:400,visibleItems: 1}, landscape: { changePoint:768,visibleItems: 2},tablet: { changePoint:991,visibleItems: 3},tablet_land: { changePoint:1199,visibleItems: 5}}});</script>';
						break;
						case 'VID':
	...
	public function hookDisplayHeader($params) {
		$this->context->controller->addCss($this->_path.'css/pk_vertflexmenu.css');
		$this->context->controller->addCss($this->_path.'css/pk_vertflexmenu_responsive.css');		
		//$this->context->controller->addJS($this->_path.'js/jquery.flexisel.js');
		$this->context->controller->addJS($this->_path.'js/pk_vertflexmenu.js');
	}
	...
	public function hookLeftColumn($params) {
		...
		$this->flexMenu();
		$this->smarty->assign('vertflexmenu', $this->_menu);
		$this->smarty->assign('this_path', $this->_path);
		return $this->display(__FILE__, 'pk_vertflexmenu.tpl');
	}
    ...
    public function installQuickAccess(){
      ...
      foreach ($languages as $language)
          $qick_access->name[$language['id_lang']]= 'Vertical Menu';
    ...
    public function getOptions()	{
		$query = 'SELECT * FROM `'._DB_PREFIX_.'pk_theme_settings'.'` WHERE id_shop = '.(int)$this->context->shop->id.' AND name = "cat_icons";';		
