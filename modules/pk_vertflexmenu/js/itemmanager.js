$(document).ready(function () {
    ...
    var exids = $('#right_product_id').val().replaceAll('PRD', '');
    ...
    var exids = $('#mainproductsitemsInput').val().replaceAll('PRD', '');
    ...
    function getProducts(pID) {
		...
		$.ajax({
		    async:false,
		    url: '../modules/pk_vertflexmenu/ajax/getimages.php',
		    data: 'pID='+pID,
		    success: function(result){
				tmp = result;
		    }		    
...
//            success: function(result){
  //              console.log(result);
	...
	function uploader(thisa, resp) {
        ...
        if (formdata) {
            $.ajax({
                url: "../modules/pk_vertflexmenu/ajax/upload.php",
                type: "POST",
                data: formdata,
                processData: false,
                contentType: false,
