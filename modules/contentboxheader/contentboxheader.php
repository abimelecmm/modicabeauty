...
class CONTENTBOXHEADER extends Module
{
	public function __construct()
	{
		$this->name = 'contentboxheader';
		$this->description = 'Content that will be added to the header of the website';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Contentboxheader_MONOLANGUAGE';
		$this->text_editor_content = 'Contentboxheader_TEXTEDITOR';
		$this->content_wrapper = 'Contentboxheader_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Contentboxheader_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Contentboxheader_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Contentbox - header');
		$this->description = $this->l('Content that will be added to the header of the website');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !CONTENTBOXHEADERModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !CONTENTBOXHEADERModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = CONTENTBOXHEADERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = CONTENTBOXHEADERModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					CONTENTBOXHEADERModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					CONTENTBOXHEADERModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																CONTENTBOXHEADERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class CONTENTBOXHEADERModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( CONTENTBOXHEADERModel::createFilesTable()
				&& CONTENTBOXHEADERModel::createContentTable());
	}
