...
if(!isset($_POST['numrecs'])) $_POST['numrecs']="250";
...
$id_shop = intval($_POST["id_shop"]);
  ...
  $infofields[$if_index++] = array("name","", "name", DISPLAY, 0, LEFT, NO_SORTER, INPUT);
  $infofields[$if_index++] = array("parent","", "parentname", DISPLAY, 0, LEFT, NO_SORTER, NOT_EDITABLE);
  $infofields[$if_index++] = array("depth","", "level_depth", DISPLAY, 200, LEFT, NO_SORTER, NOT_EDITABLE);
  $infofields[$if_index++] = array("position","", "position", DISPLAY, 0, RIGHT, NO_SORTER, NOT_EDITABLE);
  $infofields[$if_index++] = array("active","", "active", DISPLAY, 0, LEFT, NO_SORTER, INPUT);
  $infofields[$if_index++] = array("description","", "description", DISPLAY, 0, LEFT, NO_SORTER, TEXTAREA);
  $infofields[$if_index++] = array("link_rewrite","", "link_rewrite", DISPLAY, 0, LEFT, NO_SORTER, INPUT);
  $infofields[$if_index++] = array("meta_title","", "meta_title", DISPLAY, 0, LEFT, NO_SORTER, TEXTAREA);
  $infofields[$if_index++] = array("meta_keywords","", "meta_keywords", DISPLAY, 0, LEFT, NO_SORTER, TEXTAREA);
  $infofields[$if_index++] = array("meta_description","", "meta_description", DISPLAY, 0, LEFT, NO_SORTER, TEXTAREA);
  $infofields[$if_index++] = array("image","", "image", DISPLAY, 0, LEFT, NO_SORTER, NOT_EDITABLE);
...
//  row = par.rowIndex - 2; // take into account the headerrows
  ...
  document.rowform.id_category.value = tblEl.rows[rowno].cells[1].firstChild.innerHTML.replace(" ","");
  ...
  { tab = document.getElementById('Maintable');
    ...
    { if(!tblEl.rows[i].cells[fieldno]) continue; 
      ...
      if((field=="meta_description"))
        tblEl.rows[i].cells[fieldno].innerHTML = '<textarea name="'+field+i+'" rows="4" cols="35" onchange="reg_change(this);">'+tmp+'</textarea>';
	  else if(field == "description")
      { tblEl.rows[i].cells[fieldno].innerHTML = '<textarea name="'+field+i+'" id="'+field+i+'" rows="4" cols="35" onchange="reg_change(this);">'+tmp+'</textarea>'
		tblEl.rows[i].cells[fieldno].innerHTML += '<div class="TinyLine"><a href="#" onclick="useTinyMCE(this, \''+field+i+'\'); return false;">TinyMCE</a>&nbsp;<a href="#" onclick="useTinyMCE2(this, \''+field+i+'\'); return false;">TinyMCE-deluxe</a></div>';
	  }
      else if(field == "active")
	  { if(tmp==1) checked="checked"; else checked="";
	    tblEl.rows[i].cells[fieldno].innerHTML = '<input type=hidden name="'+field+i+'" id="'+field+i+'" value="0" /><input type=checkbox name="'+field+i+'" id="'+field+i+'" onchange="reg_change(this);" value="1" '+checked+' />';
	  }
	  else
        tblEl.rows[i].cells[fieldno].innerHTML = '<input name="'+field+i+'" value="'+tmp.replace("'","\'")+'" onchange="reg_change(this);" />';
    }
...
{ var submitted=0;
  ...
  { divje = document.getElementById('trid'+i);
	...
	if(docfield && (docfield.parentNode.childNodes[0].tagName == "DIV"))
...
/* the following section is for Mass Edit */
	...
	{ echo '
	  myarray["name"] = 				[1,1,1,1,0,0,0];
	  myarray["description"] = 			[1,1,1,1,0,0,1];	  
	  myarray["meta_title"] = 			[1,1,1,1,0,0,0];
	  myarray["meta_keywords"] = 		[1,1,1,1,0,0,0];
	  myarray["link_rewrite"] = 		[1,1,1,1,1,0,0];
	  myarray["meta_description"] = 	[1,1,1,1,0,0,1];';
	}
	else
	{ echo '
	  myarray["name"] = 				[1,1,1,1,0,1,0];
	  myarray["description"] = 			[1,1,1,1,0,1,1];	  
	  myarray["meta_title"] = 			[1,1,1,1,0,1,0];
	  myarray["meta_keywords"] = 		[1,1,1,1,0,1,0];
	  myarray["link_rewrite"] = 		[1,1,1,1,1,1,0];
	  myarray["meta_description"] = 	[1,1,1,1,0,1,1];';
	}
	echo '
	  myarray["active"] = 		[1,0,0,0,0,0,0];
...
';
  ...
  if($_POST['id_lang'] == "")
	...
	echo 'Search: <input name=search_txt value="'.$_POST['search_txt'].'"> &nbsp; &nbsp; ';
	echo 'Startrec: <input size=3 name=startrec value="'.$_POST['startrec'].'">';
	echo ' &nbsp &nbsp; Number of recs: <input size=3 name=numrecs value="'.$_POST['numrecs'].'">';
	echo '</form>';
	
	/* ***********************************MASS UPDATE********************************************* */
	
	$field_array = array("name", "active", "description", "link_rewrite", "meta_title", "meta_keywords", "meta_description");
...
//id_category  id_lang 	name 	description 	link_rewrite 	meta_title 	meta_keywords 	meta_description
  ...
  $query="select c.*,l.*,m.name AS parentname from ". _DB_PREFIX_."category c";
  $query .= " LEFT JOIN ". _DB_PREFIX_."category_lang l on l.id_lang='".mysqli_real_escape_string($conn, $id_lang)."' AND c.id_category=l.id_category AND l.id_shop='".$id_shop."'";
  ...
  if($_POST['search_txt'] != "")
    $query .= " WHERE l.name LIKE '%".$_POST['search_txt']."%' OR l.description LIKE '%".$_POST['search_txt']."%'";
...
>";
  ...
  echo mysqli_num_rows($res)." categories";
  echo '<form name="Mainform" method=post>';
  echo '<input type=hidden name=id_shop value='.$id_shop.'><input type=hidden name=verbose>';
  echo '<input type=hidden name=startrec><input type=hidden name=numrec>'; 
...
>';
  ...
  { 
    ...
    for($i=1; $i< sizeof($infofields)-1; $i++)
	...
	get_image_extension($datarow[$infofields[1][2]], "category");
    if(file_exists('../img/c/'.$datarow[$infofields[1][2]].$selected_img_extension))
      echo '<td><img src="../img/c/'.$datarow[$infofields[1][2]].$selected_img_extension.'" width="45px" height="45px" /></td>';
...
>";
  ...
  echo '<input type=hidden name=verbose><input type=hidden name=id_row><input type=hidden name=id_category></form></div>';
