    ...
    <form method="post" action="{$slider.postAction}" enctype="multipart/form-data">        
        <div id="new">
            <div class="title input">
                ...
                <input type="text" name="title" size="41" class="ghost-text tooltip2" value="{l s='The title of the slide'}" title="{l s='This will be the title on the slide.'}" /> 
            </div>
            <div class="url input">
                <label>{l s='Url'}</label>
                <input type="text" name="url" size="41" class="ghost-text tooltip2" value="{l s='Link of the slide'}" title="{l s='ex. http://myshop.com/promotions'}" />           
            </div>
            <div class="target">
                <label>{l s='Blank target'}</label>
                <input type="checkbox" name="target" class="tooltip2" value="1" title="{l s='Check this if you want to open the link in new window.'}" />
            </div>
            <div class="image input">
                <label>{l s='Image'}</label>
                <input type="file" name="image" size="29" id="image-chooser" class="tooltip2" title="{l s='Choose an image, only .jpg, .png, .gif are allowed.'}" />
            </div>
            <div class="imageName input">
                <label>{l s='Image name'}</label>
                <input type="text" name="imageName" size="41" class="ghost-text tooltip2" value="{l s='Image name'}" title="{l s='Optional! The name of the uploaded image without extension. The white spaces will be replaces with underscore ( _ )'}" />           
            </div>
            {if $slider.options.single == 1}
                <div class="language">
                    <label>{l s='Language'}</label>
                    <select name="language" class="tooltip2" title="{l s='The language of the slide.'}">
            ...
            <div class="alt input">
                ...
                <input type="text" name="alt" size="41" class="ghost-text tooltip2" value="{l s='An alternate text for the image'}" title="{l s='The image alt, alternate text for the image'}" />
            </div>
            <!--<div class="caption"> 
                <label>{l s='Caption'}</label>
                <textarea type="text" name="caption" cols=40 rows=6 class="ghost-text tooltip2" title="{l s='Be carefull, too long text isnt good and FULL HTML is allowed.'}">{l s='The slide text'}</textarea>                      
            </div>-->
