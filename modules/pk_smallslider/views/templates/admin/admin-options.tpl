		...
		<div id="options" class="container">
            <!--<div class="animation">
                ...
                <div class="select">      
                    <div class="first_select">
                        <label>{l s='Unused effects'}<span class="info_ico tooltip2" title="{l s='These are the animation effects, choose one or more and click to the Add button.'}"></span>  </label>
                        <select multiple="multiple" id="select1" name="nivo_effect[]" >
						    {foreach from=$slider.options.effect item=effect}
							    <option>{$effect}</option>
						    {/foreach}
				        </select>	
                        <input name="left2right" value="{l s='Add'}" type="button" id="add" class="green tooltip2" title="{l s='Click to add effect'}">
					    
                    </div>
                    <div class="second_select">
                        <label>{l s='Used effects'}<span class="info_ico tooltip2" title="{l s='These are the used animation effects, you can select and remove them, if its empty then all will be used ( random ).'}"></span></label>
                        <select multiple="multiple" id="select2" name="nivo_current[]" >
						    {foreach from=$slider.options.current item=current}
							    <option>{$current}</option>
						    {/foreach}
					    </select>	
                        <input name="right2left" value="{l s='Remove'}" type="button" id="remove" class="green tooltip2" title="{l s='Click to remove effect'}">
                    </div>                   
                </div>    
            </div>-->
            <div class="slice">
            ...
            <div class="configuration">
                ...
                <div>
					...
					<input type="text" name="pause" value="{$slider.options.pause}" class="tooltip2" title="{l s='How long each slide will be shown in miliseconds (default is 3 sec)'}">
                </div>
                <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='Pause the slider on mouse hover.'}"></span>
                </div>
                <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='Turn it ON if you dont want the slider to auto slide.'}"></span>
                </div>                    
            </div>
            <div class="navigation">
                ...
                <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='This controls the navigation, by default these are the litle dots under the slider.'}"></span>
                </div>  
            </div>
            <div class="other">
                <h3>{l s='Other'}</h3>
                <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='Turn it ON if you want the slider to start with a random slide.'}"></span>
                </div>
                <div>
                ...
                <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='Turn on if you want to use different slides for different languages, otherwise the default language slides will be used for all the languages.'}"></span>
                </div>       
                <div>
               ...
               <!-- <div class="switch">
					...
					<span class="info_ico tooltip2" title="{l s='Turn on if you want to use the Minic Slider only on the home page.'}"></span>
                </div>   -->
            </div>
