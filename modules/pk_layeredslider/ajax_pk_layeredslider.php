...
include_once('pk_layeredslider.php');

$minicSlider = new pk_layeredslider();
...
if (Tools::getValue('action') == 'updateOrder' && Tools::getValue('slides')){  
	...
	foreach ($slides['order'] as $key => $slide){		
		...
		if(Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'layered_slider` SET id_order = '.$i.' WHERE id_slide = '.$id[0]))
			$success = true;    
		else
			$success = false;    
	}
