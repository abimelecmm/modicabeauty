...
class pk_layeredslider extends Module
{
	protected $maxImageSize = 1048576;

	public function __construct()
	    {
		    $this->name = 'pk_layeredslider';
		    ...
		    $this->displayName = $this->l('Layered Slider');
		    $this->description = $this->l('Layered image slider for advertising.');

		    $this->table_slider = _DB_PREFIX_.'layered_slider';
		    $this->table_option = _DB_PREFIX_.'layered_slider_options';
	    }
	
	private function installDB()
		{
	
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'.$this->table_slider.'`');
    		Db::getInstance()->Execute('DROP TABLE IF EXISTS `'.$this->table_option.'`');
	
			if (!Db::getInstance()->Execute('
				CREATE TABLE `'.$this->table_slider.'` (
			...
			if (!Db::getInstance()->Execute('
				CREATE TABLE `'.$this->table_option.'` (
	...
	public function uninstall()
		{
			$image = Db::getInstance()->ExecuteS('SELECT image FROM `'.$this->table_slider.'`');
			
			//foreach($image as $img)
			//	$this->_deleteImages($img['image']);
	
			if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'.$this->table_slider.'`') OR
	    		!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'.$this->table_option.'`') OR
				!parent::uninstall())
				return false;
			return true;	
		}
	private function insertOptions()
		{
			...
			foreach($activeLanguages as $lang) {
				$sql = "INSERT INTO `".$this->table_slider."` (`id_shop`, `id_lang`, `id_order`, `lang_iso`, `title`, `url`, `target`, `image`, `subimages`, `subimages_sett`, `slide_text_sett`, `text_width`, `alt`, `caption`, `active`) VALUES
				(".$id_shop.", ".$lang['id_lang'].", 1, '".$lang['iso_code']."', '', '#', 0, 'slide01_bg.jpg', 'slide01_item01.png,slide01_item02.png,slide01_item03.png', '0{before[duration:.5,rotation:0,top:100,left:0,opacity:0]in[duration:.5,rotation:0,top:0,left:0,opacity:1]after[duration:.5,rotation:0,top:100,left:0,opacity:1]}1{before[duration:0.5,rotation:0,top:100,left:18,opacity:0]in[duration:0.5,rotation:0,top:0,left:18,opacity:1]after[duration:0.5,rotation:0,top:100,left:18,opacity:0]}2{before[duration:0.5,rotation:0,top:100,left:36,opacity:0]in[duration:0.5,rotation:0,top:0,left:36,opacity:1]after[duration:0.5,rotation:0,top:100,left:36,opacity:0]}', 'before[duration:.5,rotation:0,top:17,left:100,opacity:0]in[duration:.5,rotation:0,top:17,left:64,opacity:1]after[duration:.5,rotation:0,top:17,left:64,opacity:0]', '34', '', '<h3 class=\"trajan\">Dress Up</h3><h4 class=\"lmroman sec_bg\">Stunning New Styles for Every Day</h4><br/>\r\n“Alysum” – is a great solution for small and medium businesses. Easy customize with “Theme settings module” embedded tool.<br/><br/>', 1),
				(".$id_shop.", ".$lang['id_lang'].", 2, '".$lang['iso_code']."', '', '#', 0, 'slide02_bg.jpg', 'slide02_item01.png,slide02_item02.png', '0{before[duration:0.5,rotation:0,top:-100,left:10,opacity:0]in[duration:0.5,rotation:0,top:0,left:10,opacity:1]after[duration:0.5,rotation:0,top:200,left:10,opacity:0]}1{before[duration:0.9,rotation:0,top:-100,left:34,opacity:1]in[duration:0.9,rotation:0,top:28,left:34,opacity:1]after[duration:0.9,rotation:0,top:200,left:34,opacity:0]}', 'before[duration:0.5,rotation:0,top:20,left:120,opacity:0]in[duration:0.5,rotation:0,top:20,left:60,opacity:1]after[duration:0.5,rotation:0,top:20,left:120,opacity:0]', '34', '', '<span class=\"italtitle second_color\">spring bags</span>\n<h6 class=\"lmroman\">Colection</h6>\n<br/>Pellentesque accumsan vitae ante ac vulputate. Interdum et males fames ac ipsum primisin faucibus.<br/><br/>', 1),
				(".$id_shop.", ".$lang['id_lang'].", 3, '".$lang['iso_code']."', '', '#', 0, 'slide03_bg.jpg', NULL, '', 'before[duration:0.5,rotation:0,top:20,left:120,opacity:0]in[duration:0.5,rotation:0,top:20,left:55,opacity:1]after[duration:0.5,rotation:0,top:20,left:120,opacity:0]', '40', '', '<span class=\"italtitle\">jump into</span>\n<h6 class=\"lmroman\">SPRING</h6>\nUpdate your look now with our latest must-haves. Nulla consequat lacus at congue fringilla. <br/><br/>', 1);";
	...
	private function _displayForm()
		{	
			...
			$id_shop = (int)$this->context->shop->id;
			$options = Db::getInstance()->getRow('SELECT * FROM `'.$this->table_option.'`');
			$slides = $subImages = $slideEffects = $slidesText = $text_width = $slidesTextP = array();
	
			foreach($activeLanguages as $lang)
				$slides[$lang['iso_code']] = Db::getInstance()->ExecuteS('SELECT * FROM `'.$this->table_slider.'` WHERE id_lang ='.$lang['id_lang'].' AND id_shop = '.$id_shop.' ORDER BY id_order ASC');
	...
	public function addCSSFiles() {
		$slides = Db::getInstance()->ExecuteS('SELECT id_slide FROM `'.$this->table_slider.'`');
	...
	private function _handleNewSlide()
		{
			...
			$id_shop = (int)$this->context->shop->id;
			$lastSlideID = Db::getInstance()->ExecuteS('SELECT id_slide, id_order FROM `'.$this->table_slider.'` WHERE id_lang = '.$id_lang.' AND id_shop = '.$id_shop.' ORDER BY id_slide DESC LIMIT 1');
	...
	private function _handleSaveNumbers() {
		...
		$data = Db::getInstance()->ExecuteS('SELECT subimages_sett FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')');
		$dataText = Db::getInstance()->ExecuteS('SELECT slide_text_sett FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')');
	...
	private function _handleLoadSlide() {
		...
		$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages_sett = "'.$data.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
		//print_r($update);echo "asdasd";
		if(!$update){
			$this->smarty->assign('error', $this->l('An error occured while saving data.'));	
			return false;			
		}
		$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET slide_text_sett = "'.$data_text.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
	...
	private function _handleEditSlide()
		{	
			...
			if(!empty($_FILES['subimage']['name'])){
				...
				$allSubImagesRAW = Db::getInstance()->ExecuteS('SELECT subimages FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.(int)Tools::getValue('slideId').')');	

				if ($allSubImagesRAW[0]["subimages"] != "") {
					$subimage = $allSubImagesRAW[0]["subimages"].",".$subimage;
				} 
				$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages = "'.$subimage.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.(int)Tools::getValue('slideId').')'));
			...
			foreach ($allLang as $key => $langData) {
				...
				$allSlides = Db::getInstance()->ExecuteS('SELECT id_slide FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langData["iso_code"].'" AND id_shop = '.$id_shop.')');	

				foreach ($allSlides as $k => $id) {
					$arrSubImages = Db::getInstance()->ExecuteS('SELECT subimages FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langData["iso_code"].'" AND id_shop = '.$id_shop.' AND id_slide = '.$id["id_slide"].')');
					$allSubImages[$langData["iso_code"]][$id["id_slide"]] = explode(",", $arrSubImages[0]["subimages"]);		

					$arrTextSett = Db::getInstance()->ExecuteS('SELECT slide_text_sett FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$langData["iso_code"].'" AND id_shop = '.$id_shop.' AND id_slide = '.$id["id_slide"].')');	
				...
				foreach ($subimagesArray as $slideId => $subimagesNames) {
					foreach ($subimagesNames as $k => $subimgName) {
						if (Tools::getValue('remove_subimage_'.$slideId.'_'.$k)) {							
							...
							$allSubImagesRAW = Db::getInstance()->ExecuteS('SELECT subimages FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$lang.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')');
							$allSubImagesSETT = Db::getInstance()->ExecuteS('SELECT subimages_sett FROM `'.$this->table_slider.'` WHERE (lang_iso = "'.$lang.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')');							
							...
							$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages = "'.$imgStr.'" WHERE (lang_iso = "'.$lang.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
							$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages_sett = "'.$removedExcessItem.'" WHERE (lang_iso = "'.$lang.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
				...
				foreach ($subimagesArray as $slideId => $subimagesNames) {
					if ((int)Tools::getValue('slideId') == $slideId) {				
						if ($subimagesNames[0] == "") {
							...
							$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages_sett = "" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
						...
						$css .= "}\n";	
						//print_r($text_toDB);
						$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET slide_text_sett = "'.$text_toDB.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));

						if(!$update){
							$this->smarty->assign('error', $this->l('An error occured while saving data.'));	
							return false;			
						}
						$update2 = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET text_width = "'.$text_width.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
				...
				if (is_array($subimagesArray)) {									
					foreach ($subimagesArray as $slideId => $subimagesNames) {
						if ((int)Tools::getValue('slideId') == $slideId) {				
							...
							$update = (Db::getInstance()->Execute('UPDATE `'.$this->table_slider.'` SET subimages_sett = "'.$toDB.'" WHERE (lang_iso = "'.$langIso.'" AND id_shop = '.$id_shop.' AND id_slide = '.$slideId.')'));
	...
	public function _handleDeleteSlide()
		{
			$id_shop = (int)$this->context->shop->id;
			Db::getInstance()->delete($this->table_slider, 'id_slide = '.(int)Tools::getValue('slideId'));
		
			if(Db::getInstance()->Affected_Rows() == 1){
				Db::getInstance()->Execute('
					UPDATE `'.$this->table_slider.'` 
	...
	public function getData($position) {
			...
			if($options['single'] == 0){
				$slides[$defLangIso] = Db::getInstance()->ExecuteS('SELECT * FROM `'.$this->table_slider.'` WHERE (id_lang ='.$defLanguages.' AND id_shop = '.$id_shop.' AND active = 1) ORDER BY id_order ASC');			
			}else
				foreach ($activeLanguages as $lang)
					$slides[$lang['iso_code']] = Db::getInstance()->ExecuteS('SELECT * FROM `'.$this->table_slider.'` WHERE (id_lang ='.$lang['id_lang'].' AND id_shop = '.$id_shop.' AND active = 1) ORDER BY id_order ASC');		
	...
	public function hookwide_top($position = '')
	{
